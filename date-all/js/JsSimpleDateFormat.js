...
(function() {
	...
	ltr.y.__extends__(Year, {
		...
		getValue: function() {
			return moment(this.dt).weekYear();
	...
	ltr.w.__extends__(ltr.D, {
		...
		getWeek: function() {
			...
			/*var iDay = this.getDay();console.log(this)
			var iWeek = Math.ceil(iDay/7);console.log(iWeek)
			iDay = iDay % 7;
			iDay = (iDay ? iDay : 7) - 1;
			return ((this.dt.getDay() < iDay) ? (iWeek+1) : iWeek);*/
			return moment(this.dt).weeks();
		},
		isValidVal: function(iVal) {
			return iVal >= 1 && iVal <= 52;
		}
	});
